...
COLORS = {
    ...
    'disabled_corridor': 'red'  # 被禁用的连廊颜色
}
...
def plot_3d_map_plotly(school_data, graph=None, display_options=None, corridor_config=None):
    ...
    if corridor_config is None:
        corridor_config = {'level2_disabled': False}
    
    # 获取2楼连廊是否禁用
    is_level2_disabled = corridor_config.get('level2_disabled', False)
        ...
        for level in building_data['levels']:
            ...
            if show_level:
                ...
                for corridor in level['corridors']:
                    ...
                    is_disabled = False
                    if is_connect and is_level2_disabled:
...
def apply_corridor_config(graph, corridor_config):
    ...
    if corridor_config.get('level2_disabled', False):
        # A-C level2 连廊
...
def main():
    ...
    corridor_config = config_reader.get_corridor_status()
    ...
    # 主导航界面
        ...
        with st.sidebar:
            ...
            with col1:
                st.metric(
                    label="检测时间",
                    value=corridor_config['timestamp'].split(' ')[1] if ' ' in corridor_config['timestamp'] else corridor_config['timestamp']
                )
            with col2:
                st.metric(
                    label="距离 (cm)",
                    value=f"{corridor_config['distance']:.1f}"
                )
            
            # 显示连廊状态
            if corridor_config['level2_disabled']:
                st.warning("🚫 人流量大，2楼连廊已关闭")
            else:
                st.success("✅ 人流量正常，2楼连廊开放")
            
            # 刷新按钮
            if st.button("🔄 刷新传感器数据", use_container_width=True):
                st.session_state['config_reader']._cache = None
                st.rerun()
